...
<img src="public/img/logo-full.png" alt="peek" width="132" />
...
peek 就是为这个场景做的：**粘贴进去，自动认出是什么语言，一次看清**。
...
- 🔍 **粘贴自动识别语言**: 粘贴代码后自动判断是什么语言并切换；拿不准时保持原样，识别到编辑器没有高亮的语言则切为纯文本并说明
...
粘贴代码时自动判断语言并切换，识别结果显示在底部状态栏。识别分三层：
...
- 模型识别得出、但 Monaco 没有对应高亮的格式（Groovy、TOML、CSV、Haskell、Erlang、日志等）
  会**切为纯文本并如实提示**。这里不能保留原来的高亮：拿 JSON 的配色去渲染 Groovy，
  颜色是错的，比完全没有高亮更容易看错。
...
| 无对应高亮 | 识别为 X，暂无对应高亮，已按纯文本显示 | 认出来了，但编辑器没有这门语言，切纯文本停下错误的着色 |
...
await window.__detectSelfTest()   // 语言自动识别，27 项
```
...
node .workbuddy/tools/cdp-selftest.js "http://localhost:3000/?selftest=1" --only=ui
```

`--only` 可取 `both`（默认）/ `format` / `detect` / `paste` / `ui`。
其中 `paste` 是**粘贴链路的 UI 级 e2e**：真的往编辑器里派发一次粘贴事件，
断言语言被自动切过去、状态栏提示出现。`ui` 是**界面外壳 e2e**（19 项）：
断言页头/工具栏/状态栏/语言选择面板/中英切换/示例加载/历史收起/新页面/清空二次确认这些
真页面上的元素在不在、点了有没有反应——自检函数只能证明「识别算法对」，
证明不了「粘贴这条链路接上了」，也证明不了「界面还在」。这些用例同时会收集
...
1. 直接粘贴代码即可：会**自动识别语言并切换**，结果显示在底部状态栏。
   也可以在状态栏右下角手动选择语言——手动选择后会暂停自动识别，重新勾选工具栏的「自动识别语言」恢复。
2. 使用工具栏中的按钮进行操作（状态栏会提示当前语言使用的格式化器）：
